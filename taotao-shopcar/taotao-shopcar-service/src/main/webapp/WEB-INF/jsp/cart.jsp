...
<html xmlns="http://www.w3.org/1999/xhtml"><head>
   ...
   <script type="text/javascript">
       function setCheck(cbID) {
          var cartid= cbID.split("-")[1];
          console.log("cartid",cartid);
          var numid='changeQuantity-'+cartid;
           console.log("numid",numid);
          if (document.getElementById(cbID).checked=="true"||document.getElementById(cbID).checked==true) {
              document.getElementById(numid).setAttribute("checked",true);

              console.log("xx",true);
          }else {
              console.log("xx",false);
              document.getElementById(numid).removeAttribute("checked");
          }
...
<div class="cart-inner">
    ...
    <form action="http://localhost:8086/order/order-cart.html" method="post">
    <div id="product-list" class="cart-tbody">
        <!-- ************************商品开始********************* -->
        <c:set var="totalPrice" value="0"></c:set>
        <c:forEach items="${cartList}" var="cart">
        	<c:set var="totalPrice"  value="${ totalPrice + (cart.totalPrice)}"/>
	        <div id="product_11345721" data-bind="rowid:1" class="item item_selected ">
		        <div class="item_form clearfix">
		            <div class="cell p-checkbox">
                        <input id="cb-${cart.id}" onchange="setCheck(this.id)" data-bind="cbid:1" class="checkbox" type="checkbox" name="checkItem" checked="true" value="${cart.id}"></div>
		            <div class="cell p-goods">
		                <div class="p-img">
		                	<a href="/item/${cart.id }.html" target="_blank">
		                		<img clstag="clickcart|keycount|xincart|p-imglistcart" src="${cart.itemImg}" alt="${cart.itemTitle}" width="52" height="52">
		                	</a>
		                </div>    
		                <div class="p-name">
		            ...
		            <div class="cell p-quantity" for-stock="for-stock-11345721">
		                <div class="quantity-form" data-bind="">
		                    ...
		                    <input style="display:none" name="num" checked="true" type="checkbox" class="quantity-text" itemPrice="${cart.price}" itemId="${cart.id}" value="${cart.num }" id="changeQuantity-${cart.id}">
                            <input checked="true" type="text" class="quantity-text" itemPrice="${cart.price}" itemId="${cart.id}" value="${cart.num }" id="changeQuantity">
          ...
          <div class="cart-total-2014">
              <div class="cart-button">
                  <span class="check-comm-btns" id="checkout-jd">
                      <input type="submit" class="checkout"  clstag="clickcart|keycount|xincart|gotoOrderInfo" id="toSettlement">去结算<b></b></input>
                  </span>
